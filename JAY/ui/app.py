...
from JAY.search_engine.vector_store import get_stats, populate_sample
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("""
**Prompt Parser & Orchestration**
- Semantic Translator: maps prompts to 600+ parameters
- LLM Reasoner: DeepSeek-R1 for scientific reasoning
- Sovereign Priority: Indian crops & climates bias
- Research Integrator: ArXiv + Semantic Scholar
- Error Recovery: Fallback models & parsing
- Explainability: Parameter choice reasoning
...
- Data cleaning pipeline
    ...
    st.divider()
    st.caption("v1.1.0 - Enhanced Orchestration")
...
with col4:
    if st.button("🥔 Potato · Himachal cold"):
        st.session_state["prompt"] = "potato varieties for Himachal Pradesh cold tolerance"
...
if run and user_prompt.strip():
    ...
    try:
        ...
        with tab_spec:
            ...
            st.markdown("**Explainability:** Parameters chosen based on:")
            st.markdown("- **Prompt Parsing:** LLM extracted crop, location, and stress factors")
            st.markdown("- **Trait Search:** Top similar traits from vector database")
            st.markdown("- **Research:** Latest papers on stress tolerance")
            st.markdown("- **Sovereign Bias:** Prioritized Indian agricultural context")
...
st.divider()
st.subheader("🔄 Multi-turn Orchestration")
refine_input = st.text_area("Refine or follow-up query:", height=60, placeholder="e.g., What about drought resistance? Or change location to Punjab.")
if st.button("🔄 Refine Spec", use_container_width=True) and refine_input.strip():
    combined_prompt = f"{user_prompt}\n\nRefinement: {refine_input}"
